...
def dataBuilder(run_number):
    ...
    CMS_noise = pd.read_csv(f'../UTCOND/NZS_data/{run_number}/CMS_noise.csv',names=['ChannelID','Signal','CMSubstracted','Int']).drop(['Int'],axis=1)

    commonMode = pd.read_csv(f'../UTCOND/NZS_data/{run_number}/commonMode.csv',names=['ChipID','ChipMean','ChipSigma','Int']).drop(['Int'],axis=1)

    pedestals = pd.read_csv(f'../UTCOND/NZS_data/{run_number}/pedestals.csv',names=['ChannelID','PedestalValue',])

    sigmaNoise = pd.read_csv(f'../UTCOND/NZS_data/{run_number}/sigmaNoise.csv',names=['ChannelID','Sigma','STD'])


    #Importing Pickle Files for Mapping
    translator_=pd.read_pickle('./Data/translator.pkl')
    universal_map_ = pd.read_pickle('./Data/universal_map.pkl')
    ...
    sector_ =  commonMode.Sector.str.split('_',expand=True)
    ...
    commonMode[['Staves', 'Rows']] = pd.DataFrame({'Staves': staves, 'Rows': rows})
