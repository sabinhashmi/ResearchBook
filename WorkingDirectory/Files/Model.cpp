...
void min_max_scale(std::vector<float>& features, const Model& model) {
...
Model load_model_from_json(const std::string& buffer) {
    Model model;
...
// --------- Inference Core ---------
float predict(const Model& model, const std::vector<float>& features) {
    float result = 0.f;
...
// --------- Public Interface ---------
void EnsureCatboostModelLoaded(const std::optional<std::string>& model_buffer_opt) {
    if (!isModelLoaded) {
        if (!model_buffer_opt) {
