...
#include "boost_model/ModelArch.h"
namespace LHCb::Pr {
  namespace {
  ...
  private:
    ...
    Gaudi::Property<std::string> m_catboostModelPath{ this, "CatboostModelPath",
                                                  // "paramfile://data/GhostProbability/JsonSciFiModel_withScaling.json",
                                                  "file:///home/hashmi/ResearchBook/Tracking/Moore/JSONModel/ParamFiles/data/GhostProbability/JsonSciFiModel_withScaling.json",       
  ...
  void HybridSeeding::makeLHCbTracks( const FT::Hits& sciFiHits, Tracks& result, unsigned int part,
    ...
    for ( const auto& track : trackCandidates[part] ) {
      ...
      const float theta = std::atan(std::sqrt(tx * tx + ty * ty)); // Not used directly
