...
#include "/afs/cern.ch/work/s/skalavan/HLT2Trigger/Notebooks/02_ModelBuilding/01Seed/SeedTrackModelBackup.cpp"
...
namespace LHCb::Pr {
  ...
  protected:
    ...
    public:
      template <int C = CASE>
      addStereo( const HybridSeeding& hybridSeeding, typename std::enable_if<C == RecoverCase>::type* = 0 );
      template <int C = CASE>
      addStereo( const HybridSeeding& hybridSeeding, TracksToRecover& tracksToRecover,
                 typename std::enable_if<C != RecoverCase>::type* = 0 );
      addStereo()                                        = delete;
  ...
  void HybridSeeding::makeLHCbTracks( const FT::Hits& sciFiHits, Tracks& result, unsigned int part,
    ...
    for ( const auto& track : trackCandidates[part] ) {
      ...
      const float rawValue = ApplySciFiCatboostModel(scaled_features);
